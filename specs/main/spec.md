...
- **Policy without agent**: REJECTED with 400 Bad Request if agent ID is invalid or missing (see Assumptions)
- **Zero/negative premiums**: REJECTED with 400 Bad Request during policy creation validation
- **Deactivated agent with unpaid commissions**: ALLOWED; agent status changes but commissions remain traceable (see User Story 1, acceptance scenario 4)
- **Concurrent payment updates**: REJECTED with 409 Conflict if version mismatch (optimistic locking with version field)
- **Premium update after commission calculation**: NOT recalculated; existing commissions remain immutable; new commissions use updated premium (see Assumptions)
...
- **FR-023**: Policy creation MUST validate agent exists; return 400 Bad Request if agent ID is invalid or missing
- **FR-024**: System MUST implement rate limiting (100 requests per minute per client) to prevent abuse
- **FR-025**: System MUST encrypt sensitive data fields (agent contact info, payment details) at rest using AES-256
- **FR-026**: System MUST enforce HTTPS/TLS for all API communications (encryption in transit)
- **FR-027**: System MUST validate JWT tokens on every API request and reject expired or invalid tokens
- **FR-028**: System MUST implement CORS policy allowing only whitelisted origins
- **FR-029**: System MUST use parameterized queries to prevent SQL injection attacks
- **FR-030**: System MUST implement CSRF protection for state-changing operations (POST, PUT, DELETE)
- **FR-031**: System MUST mask or redact PII (Personally Identifiable Information) in logs and error messages
- **FR-032**: System MUST maintain data retention policies compliant with GDPR (right to be forgotten)
- **FR-033**: System MUST store API keys and JWT secrets in secure vault (environment variables, not hardcoded)
...
- **SC-011**: Rate limiting is enforced: 100 requests per minute per client; excess requests return 429 Too Many Requests
- **SC-012**: All sensitive data (passwords, API keys, PII) is encrypted at rest using AES-256 encryption
- **SC-013**: All API communications use HTTPS/TLS with minimum TLS 1.2; HTTP requests are rejected
- **SC-014**: JWT tokens are validated on every request; expired tokens return 401 Unauthorized
- **SC-015**: CORS policy is configured to allow only whitelisted origins; unauthorized origins return 403 Forbidden
- **SC-016**: All database queries use parameterized statements; no dynamic SQL concatenation
- **SC-017**: CSRF tokens are required for state-changing operations; missing tokens return 403 Forbidden
- **SC-018**: PII is masked in logs and error messages; no sensitive data exposed in responses
- **SC-019**: Data retention policies comply with GDPR; user data can be deleted on request
- **SC-020**: API keys and secrets are stored in secure vault (environment variables); no hardcoded secrets in code

## Assumptions

- Commission tiers are stored as percentages (e.g., 10 for 10%)
- Policies are always linked to exactly one agent at creation time
- **Policy creation MUST fail with 400 Bad Request if agent ID is invalid or missing** (clarified from edge case)
- Payments are always linked to exactly one commission
- **Existing commission records are immutable; premium updates only affect future commissions** (clarified from edge case)
- **Payment updates use optimistic locking with version field to prevent concurrent conflicts** (clarified from edge case)
